...
def get_ylabels(meta_total):
    ...
    for i, perc in enumerate(aux):
        aux[i] = locale.format('%.0f', int(perc * meta_total / 100), True) # ATENÇÃO! Diferença no R$ ao original
...
def gerar_imagem_vl_loja(fun):
    ...
    fig.savefig('./vl/grafico_loja/{}.png'.format(str(cd_fun)), bbox_inches='tight')
    plt.close(fig)
