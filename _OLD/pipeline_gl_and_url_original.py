...
df_piloto = pd.read_excel('./inputs/ad_hoc/premiacao_diaria_piloto/Mod. de Premiação - Perfis e Portes de Loja v07_MAR-2019.xlsx', sheet_name='BD FILIAIS')
...
df_rot = pd.read_csv('./inputs/de_paras/roteirizacao.csv', encoding='latin-1')
...
for fun in df_loja.itertuples():
    format_dict = {'cd_fun': fun.CD_FUN_GER,
        'cd_fil': fun.CD_FIL,
    ...
    msg_url = TEMPLATE_URL.format(**format_dict)
    
    df_loja.at[fun.Index,'url'] = msg_url.rstrip()

print(df_loja.loc[randint(0,df_loja.shape[0] - 1),'url'])

print(df_loja.loc[randint(0,df_loja.shape[0] - 1),'message'])

df_loja.columns

df_loja_output = df_loja.copy()

df_loja_output['CD_FUN_GER'] = df_loja_output.CD_FUN_GER.astype(int) + 2100000000

df_loja_output['CD_FUN_GER'] = df_loja_output['CD_FUN_GER'].astype(str)

df_loja_output = df_loja_output.rename(columns = {'CD_FUN_GER':'username'})

df_loja_output.head()

df_print = df_loja_output[['username','message','url']]

df_print.head()

df_print = get_espelhamento(df_print,df_esp,'GL')

df_print.tail(30)

# df_print.to_csv('./outputs/to_be_sent/{}/premio_mensal_gl_{}.csv'.format(DATE_FILE,DATE_FILE), index = False)
